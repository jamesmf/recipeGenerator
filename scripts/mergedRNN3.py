...
cv       = CountVectorizer(min_df=word_thr)
...
contextVecs     = [np.append(ldaVecs[i],ingNums[i]) for i in range(0,len(recipes))]
...
if len(sys.argv) > 1:
    ...
    contextModel.add(Dense(512,batch_input_shape=(batchSize,vecSize)))
...
while True: 
    ...
    sortInds    = sorted(range(0,batchSize),key=lambda x: len(recs[x]),reverse=True)
    contextBatch= np.array([Xcontext[n] for n in sortInds])
    ...
    for i in range(0,Xchar.shape[0]/batchSize):
        ...
        loss    = model.train_on_batch([xcharbat,xwordbat,contextBatch], ybat)
    ...
    if (ind / batchSize) % 50 == 0:
        ...
        with open("../models/recipeRNN3.json",'wb') as f:
            f.write(jsonstring)
        model.save_weights("../models/recipeRNN3.h5",overwrite=True)